...
async function loadSubscriptionStatus(employerId) {
    try {
        const response = await fetch(`http://localhost:5000/api/employers/subscription-status/${employerId}`, {
...
async function loadEmployerJobs(employerId) {
    try {
        const response = await fetch(`http://localhost:5000/api/employers/jobs/${employerId}`, {
            headers: { 'Authorization': `Bearer ${getAuthToken()}` }
        });
        const data = await response.json();
...
async function postNewJob(event) {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/employers/post-job', {
            method: 'POST',
            headers: {
